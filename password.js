...
var stringPasswordLength= stringPassword.length;
//I have declared a new variable, so I can later validate my password length by the number of characters
//I'm using an if statement to validate my user input for the password. This will make sure the password given starts a long enough length. 
 if (stringPasswordLength<8)
 {
    stringOutput="I'm sorry. Your password is too short. Please enter a new one.";
     $("output").value= stringOutput; 
    $("password").value= "";
    //this makes sure the potential password is not less than or equal to 8 characters and clears out the input box to try again  
 }
 
 var intCount= stringPasswordLength;
 // I'm declaring intCount as the password length, so I can decrement that in my loop
 while (intCount>=8)
 //I'm using a while loop and decrementing it based on the length of the password
       {
         if(stringPassword === arrayBadPasswords)
         //If the password enteres is the same as one of the bad passwords in the bad password array, I will tell the user to try another password
         {
            stringOutput= "Error. This password is not an acceptable password. Please try again.";
            $("output").value= stringOutput;
            //this puts the output message pop up on the screen
            break;
                 //once this loop is checked, I'm breaking out of it 
         }
         else {
              //as long as the password is greater than or equal to 8 characters and doesn't equal a "bad password" the user will be congratulated 
            stringOutput= "Congrats! Your password is perfect." + " " + stringPassword;
             $("output").value= stringOutput; 
         }
        intCount--;
        //I'm decrementing my loop because people make their passwords as long as they want
